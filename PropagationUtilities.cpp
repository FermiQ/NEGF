                                                  ...
                                                  PetscMatrixParallelComplex*& result)
{
  ...
  {
    //===============================================
    // this is old version -- super matrix concept
    //-----------------------------------------------
  ...
  else
  {
    ...
    //=====================================
    // new version -- avoid super matrix
    //-------------------------------------
    ...
    DOFmapInterface*& subdomain_coupling_DOF)
{
  ...
  //std::set<unsigned int>* pointer_to_Hamilton_momentum_indices=&Hamilton_momentum_indices;
  //find_Hamiltonian_momenta(writeable_Propagators.begin()->second,pointer_to_Hamilton_momentum_indices);
  //set_valley(output_Propagator, momentum, Hamilton_Constructor); //Hamilton_Constructor 
  //set_valley(output_Propagator, momentum, Hamilton_Constructor_temp); //additional set_valley 
  //NemoMeshPoint temp_NemoMeshPoint(0,std::vector<double>(3,0.0));
...
//12/09/2012 -  - Creating this placeholder that can be used if full submatrices need to be created
...
//11/9/2012 - : creating this function to initialize some of the matrices involved, esp Wave_left and expmi_delta_left as full.
    ...
    PetscMatrixParallelComplex*& T43, PetscMatrixParallelComplex*& M1_matrix, PetscMatrixParallelComplex*& M2_matrix)
{
  ...
  //  number_of_EmH_rows, nz_per_owned_P_rows_diagonal, nz_per_owned_P_rows_offdiagonal, P_matrix); // if solve M=inv(HP)*P
  transfer_matrix_initialize_temp_full_matrix(this_simulation,number_of_EmH_rows, number_of_P_cols,
      nz_per_owned_P_rows_diagonal, nz_per_owned_P_rows_offdiagonal, P_matrix); // if solve HP*M=P
    ...
    unsigned int* number_of_eigenvalues, unsigned int* number_of_vectors_size, unsigned int* num_left, unsigned int* num_right)
{
  ...
  *num_right = 0;
  ...
  //===========================================================24
  // copy eigenvectors to petscmatrix, do matrix-matrix mult
  //-----------------------------------------------------------
  ...
  for(unsigned int i=0; i<num_eig_take; i++)
  {
    ...
    if(cond) //if not the osolete solution phase factor !=0 or 1
    {
      ...
      kdelta[i]=cplx(0.0,1.0)*log(expmikdelta[i]); // correct version
      ...
      if((kdelta[i].imag()<imag_limit)&&(kdelta[i].imag()>-imag_limit))  //OMEN--Injection::get_type()
      {
        ...
        velocity=2*velocity_temp.imag();//velocity of waves -- : change - to + in front
  ...
  *number_of_vectors_size=number_of_long_vector;
  ...
  vector<int> nz_per_owned_exp_left_rows_diagonal_velocity(*num_left,0); // commented above two lines and inserted these
  ...
  transfer_matrix_initialize_temp_matrix(this_simulation,*num_left,*num_left,
                                         nz_per_owned_exp_left_rows_diagonal,nz_per_owned_exp_left_rows_offdiagonal,out_of_device_phase);// this is diagonal matrix, should not be full
  ...
  vector<int> nz_per_owned_exp_right_rows_diagonal_velocity(n_right,0); // commented above two lines and inserted these
  ...
  transfer_matrix_initialize_temp_matrix(this_simulation,n_right,n_right,
                                         nz_per_owned_exp_right_rows_diagonal,nz_per_owned_exp_right_rows_offdiagonal,into_device_phase); // this is diagonal matrix, should not be full
  ...
  transfer_matrix_initialize_temp_full_matrix(this_simulation,*number_of_vectors_size,n_left_propagating,nz_left_propagating_wave_rows_diagonal,
      ...
      out_of_device_propagating_modes); // make it dense, seems necessary for avoiding reordering
  transfer_matrix_initialize_temp_full_matrix(this_simulation,*number_of_vectors_size,n_right_propagating,nz_right_propagating_wave_rows_diagonal,
      ...
      into_device_propagating_modes); // make it dense, seems necessary for avoiding reordering
  ...
  //: the two lines above have incorrect total matrix sizes (first two arguments).
  transfer_matrix_initialize_temp_matrix(this_simulation,*num_left,*num_left,
    nz_per_owned_exp_left_rows_diagonal_velocity,nz_per_owned_exp_left_rows_offdiagonal_velocity,out_of_device_velocity);
  transfer_matrix_initialize_temp_matrix(this_simulation,n_right,n_right,
    nz_per_owned_exp_right_rows_diagonal_velocity,nz_per_owned_exp_right_rows_offdiagonal_velocity,into_device_velocity);
  ...
  {//: sort propagating modes by velocities and decaying by phase factor's magnitude. If phase factor's magnitude is equal then by phase factor's phase.
  ...
  PetscMatrixParallelComplex*& result)
{
  ...
  temp_self->matrix_convert_dense(); // convert to dense for efficient mult
    ...
    PetscMatrixParallelComplex*& result)
{
  ...
  std::vector<int> temp_cols(number_of_sub_rows); // assuming gamma is square block
  ...
  sub_Gamma= new PetscMatrixParallelComplex(number_of_sub_rows,number_of_sub_rows,
      this_simulation->get_simulation_domain()->get_communicator() );
  //sub_Gamma->consider_as_full(); // dense block -- does not affect get_submatrix
  ...
  // petsc does not provide a method for it, so we need to do it ourselves
  sub_Gamma->matrix_convert_dense();

  //================================
  // II extract submatrix of last block of GR
  ///-------------------------------
                                                     ...
                                                     std::vector<std::string>& Mesh_tree_names, std::map<NemoMesh*, NemoMesh* >& Mesh_tree_downtop)
{
  ...
  // Hack for transverse k parallelization. Instead of doing 1D band structure multiple times in serial, do 2D or 3D once by using
