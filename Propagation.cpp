...
Propagation::~Propagation()
{
  ...
  if(disable_meshclean==false&&produced_a_mesh)//changed to reduce string operation 
  {
    ...
    if(momentum!=NULL)
    {
      ...
      if(it->second)
      {
        ...
        if(it2->second!=NULL)
        {
          ...
          {
            //// this is used for free_memory_of combine solver
...
void Propagation::calculate_spatial_current()
{
  ...
  // fill the integrated current density
...
{
  // if current conserves, then the summed current over each atom is zero, except for the atoms at the device/contact interface
...
void Propagation::calculate_nonrectangular_mesh_current()
{
  ...
  std::set<std::vector<double> > temp_all_kvectors; // (k-resolved current)
  ...
  for(c_it=pointer_to_all_momenta->begin(); c_it!=pointer_to_all_momenta->end(); c_it++)
  {
    ...
    if(transmission_cit!=momentum_transmission.end())
    {
      ...
      if(!valley_only)
      {
        // (k-resolved current without k integration weights)
...
void Propagation::clean_all()
{
  ...
  bool was_this_parallelizer = parallelized_by_this(); // Look a few lines below for explanation.
  ...
  // modified mesh deletion. First, take into account that mesh might already be deleted by external constructor so do not attempt to find mesh name
  ...
  if(get_energy_resolved_data)
  {
    ...
    MPI_Comm_rank(topcomm,&my_rank);
    {
      ...
      if(my_rank==0&&options.get_option(name_of_propagator+"_energy_resolved_output",false))
      {
        ...
        {
          std::string filename = "energy_resolved_"+name_of_propagator; //this should be enough, otherwise the name will be too long
          //if(options.check_option("output_suffix"))
  ...
  if(options.get_option(name_of_propagator+"_k_resolved_output",false) || k_resolved_density || options.get_option("k_resolved_density",false))
  {
    ...
    MPI_Comm_rank(topcomm,&my_rank);
    {
      ...
      if(my_rank==0 && options.get_option(name_of_propagator+"_k_resolved_output",false))
      {
        ...
        {
          std::string filename = "k_resolved_"+name_of_propagator; //this should be enough, otherwise the name will be too long
          //if(options.check_option("output_suffix"))
          //  filename += options.get_option("output_suffix",std::string(""));
