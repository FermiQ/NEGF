...
void Schroedinger::base_reinit()
{
  ...
  //Hack to allow change in k_points for E(k) grid when do_solve of band structure is called from adaptive grid for each k 
  ...
  {//hack
    ...
    {
      //Allow reading a vector of k_points directly (no LIBMESH grid generation) 
      if (!options.check_option("k_points_vector"))
      {
        int number_of_k_points = options.get_option("number_of_k_points",10);

        //Allow different number of points in different directions 
	...
	Mesh mesh(Parallel::Communicator(MPI_COMM_SELF));
        ...
        //Allow input in Cartesian coordinates 
        if (k_space_basis!="cartesian")
        {
        ...
        for (int i=0; i<n; i++)
        {
          ...
          if (k_space_basis!="cartesian") //If cartesian is input, no need to transform 
          {
  ...
  // is the electron / hole discrimination threshold set autmatically by NEMO?
...
void Schroedinger::do_solve()
{
      ...
      || density_of_states || spatial_density_of_states)
  {
    ...
    // <ss> according to  2pi factors cancel out
...
void Schroedinger::set_eigensolver_options()
{
  ...
  if (eigen_values_solver=="lanczos" || eigen_values_solver=="block_lanczos" || eigen_values_solver == "lanczos_then_krylovschur")
  {
    ...
    // - why are we using ints for options instead of strings?
  ...
  else
  {
    ...
    //set-up ordering (This block of code used to be below, but was moved up to this location )
    ...
    else
    {
      ...
      if (_automatic_threshold)
      {
        ...
        else if (calculate_band_structure && eig_solver.strict_threshold && (eig_solver.sort_like == "electrons")) //option 
        {
          shift = _electron_threshold_energy;
          if (eigen_values_solver == "arpack")
          {
            eig_solver.set_transformation_type("sinvert"); //for krylovschur this can be "none", but seems faster with "sinvert" 
        ...
        else if (calculate_band_structure && eig_solver.strict_threshold && (eig_solver.sort_like == "holes")) //option 
        {
...
// : Get spatial threshold or charge sign
void Schroedinger::get_data(const string& variable,const double& Energy, map<unsigned int, double>& result_map, map<unsigned int, double>& result_map_right)
{
  ...
  // : Spatial sign according to spatial threshold
...
void Schroedinger::get_data(const std::string& variable, const NemoMeshPoint& momentum, double& result)
{
  ...
  if(variable=="integration_weight")
  {
    ...
    {
      //For custom and OMEN like k grid. Works with single periodic direction for now (UTB). 
      double kxmin = options.get_option("kxmin", 0.0);
      double kxmax = options.get_option("kxmax", 1.0);
      //For benchmarking with OMEN 
      ...
      //Custom inhomogeneous transverse k grid 
  ...
  else if (electron_density || hole_density || density_of_states || spatial_density_of_states)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        //rectangular with multi-element. Keep backward compatibility .
        ...
        integrator.set_order(integration_order);
        integrator.build_integral_sum(); //Gaussian quadrature by default. First order Gaussain is just simple rectangular .

        vector<vector<double> > x = integrator.get_points();

        //If required avoid using libmesh generation with reciprocal space and generate k points like done below for cartesian space .
        if (options.get_option("OMEN_style_transverse_k_points", false))
        {
          int temp_size = 0;
          if(rec_lattice.size()==1 ||  (k_rotational_symmetry && rec_lattice.size()==2))
...
// added this dummy implementation.
...
void Schroedinger::set_input_options_map()
{
  ...
  set_input_option_map("Neumann_BC",InputOptions::NonReq_Def("false",
      "Set Neumann BC for hamiltonian, which can generate a good basis for EM LRA, implemented")); //
